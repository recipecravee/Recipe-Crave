// Seed recipes + collections into Supabase Postgres.
// Reads from the same in-repo seed data the app currently serves.
...
// Register tsx loader so we can import .ts seed file
register('tsx/esm', pathToFileURL('./'));

const __dirname = dirname(fileURLToPath(import.meta.url));
const { SEED_RECIPES, SEED_COLLECTIONS } = await import(
  pathToFileURL(join(__dirname, '..', 'src', 'content', 'seed-recipes.ts')).href
);
...
try {
  ...
  for (const c of SEED_COLLECTIONS) {
    // Resolve recipe slugs to actual UUIDs from DB
  ...
  const [{ count: collectionCount }] = await sql`SELECT COUNT(*)::int AS count FROM collections`;
  console.log(`\n📊 DB totals: ${recipeCount} recipes, ${collectionCount} collections`);
