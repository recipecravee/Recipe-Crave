    ...
    <div className="grid gap-8 lg:grid-cols-[1fr,1.5fr]">
      ...
      <div>
        {!result ? (
          ...
          <div id="meal-plan-printable" className="space-y-6">
            {/* Print-only brand header — invisible on screen, prints at top of every page. */}
            ...
            <div className="print-keep-colors rounded-2xl bg-forest-500 p-6 text-white">
              <p className="text-sm uppercase tracking-wider opacity-80">Your personalized week</p>
              <p className="mt-1 font-serif text-3xl">
                ${result.totalCostUsd.toFixed(2)} · {result.plan.length} meals
              </p>
              <p className="mt-1 text-xs opacity-80">
                Household of {householdSize} · {diet ? diet : 'no dietary filter'} ·
                budget ${weeklyBudgetUsd.toFixed(0)}/wk
                {pantry.trim() ? ` · pantry: ${pantry}` : ''}
              </p>
            </div>
            ...
            {/* Print-only footer. Visible only when the user prints / saves as PDF. */}
            <div className="print-brand-footer">
              Generated by RecipeCrave · recipecrave.com · Free AI meal planning ·
